...
class GraphVAE(nn.Module):
    ...
    def __init__(
        ...
        self.decoder_trans_dim=decoder_trans_dim
        self.epoch = 0
        self.step = 0
        # DWA
        if lambdas is None:
            self.lambdas = np.array([10, 100, 0.25, 50, 0.1])  # V, P,Comm,Pre
        else:
        ...
        self.decoder_lattice_dims2 = decoder_lattice_dims2
        self.decoder_lattice_dims2.insert(0, self.latent_dim * 2)
        self.decoder_lattice2 = nn.ModuleList()
        for i in range(len(self.decoder_lattice_dims2) - 1):
            self.decoder_lattice2.append(
                nn.Sequential(
                    nn.Linear(
                        self.decoder_lattice_dims2[i], self.decoder_lattice_dims2[i + 1]
                    ),
                    nn.Dropout(0.1),
                    nn.LeakyReLU(0.1),
                )
            ...
            self.decoder_lattice2.append(
                nn.TransformerEncoderLayer(
                    self.decoder_lattice_dims2[i + 1], 4, 256, batch_first=True
                )
            )
        self.decoder_lattice2.append(
            nn.Sequential(nn.Linear(self.decoder_lattice_dims2[-1], 6))
        )
        self.decoder_lattice1 = nn.Sequential(
        ...
        self.decoder_trans_dim.insert(0, self.input_dim)
        self.decoder_trans = nn.ModuleList()
        for i in range(len(self.decoder_trans_dim) - 1):
            self.decoder_trans.append(
                nn.Sequential(
                    nn.Linear(self.decoder_trans_dim[i], self.decoder_trans_dim[i + 1]),
                    nn.Dropout(0.1),
                    nn.LeakyReLU(0.1),
                )
            )
            self.decoder_trans.append(
                nn.TransformerEncoderLayer(
                    self.decoder_trans_dim[i + 1], 4, 256, batch_first=True
                )
            )
        self.decoder_trans.append(
            nn.Sequential(
                nn.Linear(self.decoder_trans_dim[-1], self.input_dim), nn.LeakyReLU(0.1)
            )
        )
    ...
    def graph_offset(self, F: Tensor, f_mask: Tensor, lattice: Tensor):
        ...
        coord_out,type_out = self.decoder_diff.forward_decode(
            atom_input, frac_coords_input, lattice, num_atoms_input, node2batch_input
        )
        coord_out=batch_crystals(coord_out,f_mask.sum(-1))+F[..., self.d1 :]
        atom_out=batch_crystals(type_out,f_mask.sum(-1))+F[..., :self.d1]
        coord_out=F[...,self.d1:]
        F_new = torch.cat([atom_out, coord_out], dim=-1)
        
        return lattice,F_new
    
    # def trans_offset(self, F: Tensor, f_mask: Tensor, lattice: Tensor):
        
    #     atom_input = Func.one_hot(F[..., : self.d1].argmax(dim=-1),num_classes=100)
    #     frac_coords_input = F[..., self.d1 :]
    #     input=torch.cat([atom_input,frac_coords_input],dim=-1)
    #     for i, net in enumerate(self.decoder_trans):
    #         if i == 0:
    #             dec1_out = net(input)
    #         else:
    #             if isinstance(net, nn.TransformerEncoderLayer):
    ...
    #     F_new=torch.cat([dec1_out,frac_coords_input],dim=-1)        
    #     return lattice,F_new
        ...
        for i, net in enumerate(self.decoder_trans):
            if i == 0:
                ...
                if isinstance(net, nn.TransformerEncoderLayer):
                    ...
                    dec1_out = net(dec1_out)
        F_new=torch.cat([dec1_out[...,:self.d1],frac_coords_input+dec1_out[...,self.d1:]],dim=-1)        
        return lattice,F_new
    ...
    def forward(
        ...
        #lattice_new,F_new=self.graph_offset(recon_x,f_mask,recon_lattice)
        # pdb.set_trace()
        lattice_loss = (
            Func.mse_loss(recon_lattice, lattice_target)
            #+ Func.mse_loss(recon_lattice2, lattice_target)
        )
        _,recon_x=self.trans_offset(recon_x,f_mask,recon_lattice)
        # pdb.set_trace()
    ...
    def structure_matcher_with_lattice(self, F_recon, f_mask, crystals, lattice_predict):
            self.step += 1
            match = 0
            B = F_recon.shape[0]
            F_recon_ = F_recon  # B, N, K
