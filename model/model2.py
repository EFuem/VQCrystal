...
class GraphVAE(nn.Module):
    ...
    def evaluation_forward(
        ...
        return {
            ...
            'rms_dist_with_lattice': rms_dist_with_lattice
        }
    ...
    def generate_decode(self, z_q, f_mask, local=True):
        if local:
            recon_x = self.decode(None, z_q, f_mask)
        else:
            recon_x = self.decode_from_global_for_generate(z_q, f_mask)
        recon_lattice1 = self.decode_lattice_from_recon(recon_x, f_mask).sum(dim=-2)
        # recon_lattice2 = self.decode_lattice_from_vq(z_q, f_mask).sum(dim=-2)
        if local:
            # recon_lattice = (recon_lattice1 + recon_lattice2) / 2
            recon_lattice = recon_lattice1
        else:
            recon_lattice = recon_lattice1
        lattice_predict = self.normalizer.denorm(recon_lattice)
    ...
    def structure_matcher_with_lattice(self, F_recon, f_mask, crystals, lattice_predict):
        ...
        for atom, position, lattice, crystal in zip(element_list, P_list, lattice_list, crystals):
            ...
            try:
                ...
                is_match = int(self.matcher.get_rms_dist(structure, crystal) is not None)
                if not is_match:
        ...
        rms_dist = rms_dists[rms_dists != None]
        return match / B, (rms_dist.sum(),len(rms_dist))
