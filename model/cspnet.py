...
from utils import lattice_params_to_matrix_torch, get_pbc_distances, radius_graph_pbc, frac_to_cart_coords, repeat_blocks
...
class CSPNet(nn.Module):
    ...
    def __init__(
        ...
        self.coords_embedding=nn.Linear(3,hidden_dim)
        self.atom_latent_emb = nn.Linear(hidden_dim*2, hidden_dim)
        self.atom_latent_emb_ = nn.Linear(hidden_dim, hidden_dim)
        if act_fn == 'silu':
            self.act_fn = nn.SiLU()
        if dis_emb == 'sin':
        ...
        self.global_features_layer=nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.SiLU(),
            nn.Linear(hidden_dim, hidden_dim)
        )
    ...
    def forward_decode(self, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        node_features = self.atom_latent_emb_(node_features)
    ...
    def forward(self, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        if self.smooth:
            ...
            node_features = self.node_embedding(atom_types - 1)
        coords_embedding=self.coords_embedding(frac_coords)
        node_features=torch.cat([node_features,coords_embedding],dim=-1)    
        ...
        return local_features,graph_features

class CSPNetDeep(nn.Module):
    ...
    def __init__(
        ...
        super(CSPNetDeep, self).__init__()

        self.ip = ip
        self.smooth = smooth
        self.node_embedding = nn.Linear(input_atom_dim, hidden_dim)
        self.coords_embedding=nn.Linear(3,hidden_dim)
        self.atom_latent_emb = nn.Linear(hidden_dim*2, hidden_dim)
        self.atom_latent_emb_ = nn.Linear(hidden_dim, hidden_dim)
        if act_fn == 'silu':
